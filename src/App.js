...
const GLOBAL_STYLES = `@import url('https://fonts.googleapis.com/css2?family=Red+Hat+Display:wght@300;400;500;600;700;800&family=Playfair+Display:wght@700;800&display=swap'); *{box-sizing:border-box;margin:0;padding:0;-webkit-tap-highlight-color:transparent;} ::-webkit-scrollbar{display:none;} body{background:#050508;} .fade-in{animation:fadeUp 0.35s ease forwards;} @keyframes fadeUp{from{opacity:0;transform:translateY(10px)}to{opacity:1;transform:translateY(0)}} @keyframes slideUp{from{transform:translateY(100%)}to{transform:translateY(0)}} @keyframes pop{from{transform:scale(0);opacity:0}to{transform:scale(1);opacity:1}} @keyframes pulse{0%,100%{opacity:1}50%{opacity:0.4}} @keyframes orbFloat{0%,100%{transform:translateY(0px) scale(1)}50%{transform:translateY(-6px) scale(1.03)}} @keyframes orbSpin{from{transform:rotate(0deg)}to{transform:rotate(360deg)}} @keyframes orbSpinRev{from{transform:rotate(0deg)}to{transform:rotate(-360deg)}} @keyframes orbPulse{0%,100%{transform:scale(1);opacity:0.7}50%{transform:scale(1.15);opacity:1}} @keyframes wave1{0%,100%{d:path("M0,50 Q25,20 50,50 Q75,80 100,50")}50%{d:path("M0,50 Q25,80 50,50 Q75,20 100,50")}} @keyframes blobMorph{0%,100%{border-radius:60% 40% 30% 70%/60% 30% 70% 40%}25%{border-radius:30% 60% 70% 40%/50% 60% 30% 60%}50%{border-radius:50% 60% 30% 60%/30% 40% 70% 50%}75%{border-radius:40% 60% 50% 40%/70% 30% 50% 60%}} input:focus,textarea:focus,select:focus{outline:none;} input::placeholder,textarea::placeholder{color:#3d3d55;} input[type=range]{-webkit-appearance:none;height:4px;border-radius:2px;background:#1e1e2e;} input[type=range]::-webkit-slider-thumb{-webkit-appearance:none;width:20px;height:20px;border-radius:50%;background:linear-gradient(135deg,#7c3aed,#a78bfa);cursor:pointer;} select{appearance:none;} @media (min-width: 768px) { .mobile-only { display: none !important; } .desktop-screen { padding-top: 40px !important; padding-bottom: 40px !important; } .desktop-screen > div:first-child { position: relative !important; top: auto !important; padding-top: 0 !important; } .desktop-two-col { display: grid !important; grid-template-columns: 1fr 1fr !important; gap: 20px !important; align-items: start !important; } .desktop-three-col { display: grid !important; grid-template-columns: 1fr 1fr 1fr !important; gap: 16px !important; } } @media (max-width: 767px) { .desktop-only { display: none !important; } }`;
...
```
 ...
 {[{ icon: " ", label: "Services", screen: "services" }, { icon: " ", label: "Payments", screen: "payments" }, { icon: " ", label: "Settings", screen: "settings" }, { icon: " ", label: "Loyalty", screen: "loyalty" }, { icon: " ", label: "Analytics", screen: "analytics" }, { icon: " ", label: "Promotions", screen: "promotions" }, { icon: " ", label: "Staff", screen: "staff" }, { icon: " ", label: "Waitlist", screen: "waitlist" }, { icon: " ", label: "Share Booking Link", screen: "sharelink" }].map(item => (
...
const details = encodeURIComponent(`Client: ${a.client_name} Phone: ${a.client_phone || ""} Price: ${a.price || ""}`);
...
if (!error && onNoteUpdate) onNoteUpdate(”**CLEAR**”);
...
{activeTab === “notes” && <NoteTab client={selectedClient} onNoteUpdate={(note) => setSelectedClient(p => ({ …p, note: note === “**CLEAR**” ? “” : (p.note ? p.note + “\n\n” : “”) + note }))} />}
{activeTab === “contact” && (
<Card>
...
```
 ...
 <div onClick={() => openEdit(s)} style={{ width: 32, height: 32, borderRadius: 9, background: C.surfaceHigh, border: `1px solid ${C.border}`, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", fontSize: 13 }}> </div>
 <div onClick={() => deleteService(s.id)} style={{ width: 32, height: 32, borderRadius: 9, background: "#f43f5e11", border: "1px solid #f43f5e22", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", fontSize: 13 }}> </div>
 </div>
 </div>
 </Card>
 ))}
...
{ icon: “ ”, label: “PayPal”, sub: “Your PayPal email or link”, connected: false, placeholder: “PayPal email or paypal.me/link” },
...
<Card>
{[
{ icon: “ ”, label: “Services”, sub: “Manage your services & prices”, screen: “services” },
...
{ icon: “ ”, label: “Email”, color: C.blue, key: “email” },
...
const today = new Date().toLocaleDateString(“en-US”, { weekday: “long”, month: “long”, day: “numeric” });
const time = new Date().toLocaleTimeString(“en-US”, { hour: “2-digit”, minute: “2-digit” });
return `You are ${aiName}, an AI business assistant inside Pocketflow. Today: ${today} at ${time}. BUSINESS DATA: ${bizContext || "Loading..."} PERSONALITY: Warm, sharp, confident. Like a brilliant friend who knows this business inside out. ${isVoice ? "VOICE MODE: 1-2 short natural sentences only. No markdown. Conversational." : "TEXT MODE: Use **bold** for key numbers/names. Max 3-4 sentences unless asked for more. No bullet spam."} NAVIGATION RULE — CRITICAL: When the user asks to go to any screen/page, you MUST start your response with "NAV:screenname" on its own line. No exceptions. FORMAT: NAV:screenname\nYour message here. SCREENS: schedule, inbox, clients, payments, analytics, promotions, loyalty, services, staff, waitlist, settings, home. EXAMPLES: User: "take me to inbox" → NAV:inbox\nHere are your messages! User: "open schedule" → NAV:schedule\nHere's your schedule! User: "go to clients" → NAV:clients\nHere are your clients! The "NAV:screenname" part is invisible to the user — it triggers navigation automatically. You remember this conversation — reference earlier messages when relevant.`;
};
